...
docker exec korrigo-db-1 pg_dump -U viatique_user viatique > /var/backups/korrigo/backup_$(date +%Y%m%d_%H%M%S).sql
...
docker exec -it korrigo-db-1 psql -U viatique_user -d viatique

# Backup manuel
docker exec korrigo-db-1 pg_dump -U viatique_user viatique > backup.sql

# Restaurer un backup
cat backup.sql | docker exec -i korrigo-db-1 psql -U viatique_user -d viatique
```
...
docker exec korrigo-db-1 pg_isready -U viatique_user -d viatique
...
docker exec korrigo-db-1 pg_dump -U viatique_user viatique > /var/backups/korrigo/manual_$(date +%Y%m%d_%H%M%S).sql

# Restaurer un backup
docker exec -i korrigo-db-1 psql -U viatique_user -d viatique < /var/backups/korrigo/backup.sql
```
...
cat /var/backups/korrigo/korrigo_db_YYYYMMDD_HHMMSS.sql | docker exec -i korrigo-db-1 psql -U viatique_user -d viatique
```
