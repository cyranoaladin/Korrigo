...
```bash
# Core Security (REQUIRED - no defaults in production)
...
```bash
# Set ADMIN_PASSWORD in environment first!
...
```bash
docker cp media_backup.tar.gz $(docker compose -f infra/docker/docker-compose.prod.yml ps -q backend):/tmp/
docker compose -f infra/docker/docker-compose.prod.yml exec backend tar -xzf /tmp/media_backup.tar.gz -C /
```
...
docker compose -f infra/docker/docker-compose.prod.yml exec -T db psql -U korrigo_user korrigo_prod < backup_before_deploy.sql
...
```bash
docker compose -f infra/docker/docker-compose.prod.yml exec backend python manage.py dbshell
```
...
*Version: prod-readiness/20260202-windsurf*
