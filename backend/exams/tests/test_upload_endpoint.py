...
# Additional tests for INDIVIDUAL_A4 mode - to be added to test_upload_endpoint.py


@pytest.mark.django_db
class TestIndividualModeValidation:
    """Test suite for INDIVIDUAL_A4 mode validation scenarios"""
    ...
    def test_batch_mode_requires_pdf_source(self, teacher_client):
        """
        BATCH_A3 mode should require pdf_source.
        Without pdf_source, should return 400.
        """
        data = {
            'name': 'Batch Exam Without PDF',
            'date': '2024-01-15',
            'upload_mode': 'BATCH_A3',
            'pages_per_booklet': 4
            # No pdf_source provided
        }
        
        response = teacher_client.post(self.upload_url, data, format='multipart')
        
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert 'pdf_source' in str(response.data).lower()
    
    def test_individual_mode_upload_corrupted_pdf_rejected(self, teacher_client):
        """
        Uploading corrupted PDF in INDIVIDUAL_A4 mode should fail gracefully.
        Should return error without creating any objects (atomicity).
        """
        exam = self.create_individual_mode_exam()
        
        corrupted_pdf = create_corrupted_pdf()
        pdf_file = create_uploadedfile(corrupted_pdf, filename="corrupted.pdf")
        ...
        from exams.models import ExamPDF, Copy
        assert ExamPDF.objects.filter(exam=exam).count() == 0
        assert Copy.objects.filter(exam=exam).count() == 0
    
    def test_individual_mode_upload_oversized_pdf_rejected(self, teacher_client):
        """
        Uploading PDF > 50MB in INDIVIDUAL_A4 mode should return HTTP 413.
        """
        exam = self.create_individual_mode_exam()
        
        large_pdf = create_large_pdf()  # Creates 51+ MB PDF
        pdf_file = create_uploadedfile(large_pdf, filename="large.pdf")
    ...
    def test_individual_mode_upload_fake_pdf_rejected(self, teacher_client):
        """
        Uploading non-PDF file (fake .pdf extension) should fail MIME validation.
        """
        exam = self.create_individual_mode_exam()
        
        fake_pdf = create_fake_pdf()
        pdf_file = create_uploadedfile(fake_pdf, filename="fake.pdf", content_type="application/pdf")
    ...
    def test_individual_mode_upload_empty_pdf_rejected(self, teacher_client):
        """
        Uploading empty PDF file should fail validation.
        """
        exam = self.create_individual_mode_exam()
        
        empty_pdf = create_empty_pdf()
        pdf_file = create_uploadedfile(empty_pdf, filename="empty.pdf")
...
@pytest.mark.django_db
class TestIndividualModeAtomicity:
    """Test suite for INDIVIDUAL_A4 mode atomicity guarantees"""
    ...
    def test_partial_failure_rollback(self, teacher_client):
        """
        If one PDF in a batch fails, entire transaction should rollback.
        Test atomicity: no partial state should persist.
        """
        exam = self.create_individual_mode_exam()
        
        # Create mix of valid and invalid PDFs
        pdf1 = create_uploadedfile(create_valid_pdf(pages=4), filename="valid1.pdf")
        pdf2 = create_uploadedfile(create_corrupted_pdf(), filename="corrupted.pdf")
        pdf3 = create_uploadedfile(create_valid_pdf(pages=4), filename="valid3.pdf")
        ...
        # CRITICAL: No objects should be created (atomicity guarantee)
        from exams.models import ExamPDF, Copy
        assert ExamPDF.objects.filter(exam=exam).count() == 0
        assert Copy.objects.filter(exam=exam).count() == 0
    
    @patch('exams.views.Copy.objects.create')
    def test_copy_creation_failure_rollback(self, mock_create, teacher_client):
        """
        If Copy creation fails, ExamPDF should also rollback (transaction atomicity).
        """
        ...
        # No ExamPDF should exist (rollback)
        from exams.models import ExamPDF
        assert ExamPDF.objects.filter(exam=exam).count() == 0


@pytest.mark.django_db
class TestIndividualModeSecurity:
    """Test suite for INDIVIDUAL_A4 mode security scenarios"""
    ...
    def test_path_traversal_prevention(self, teacher_client):
        """
        Uploading file with path traversal characters should sanitize filename.
        """
        exam = self.create_individual_mode_exam()
        ...
        # Verify uploaded file doesn't contain path traversal
        from exams.models import ExamPDF
        ...
        assert 'exams/individual/' in exam_pdf.pdf_file.name
    
    def test_student_cannot_upload(self, api_client):
        """
        Students should not be able to upload individual PDFs.
        Only teachers and admins have permission.
        """
        from django.contrib.auth import get_user_model
        from core.auth import create_user_roles
        
        User = get_user_model()
        _, _, student_group = create_user_roles()
        
        student = User.objects.create_user(username='student_test', password='pass')
