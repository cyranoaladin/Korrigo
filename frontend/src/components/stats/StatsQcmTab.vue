...
<template>
  ...
  <div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        ...
        <p class="text-2xl font-bold text-neutralDark">
          {{ qcmData.total_perfect }}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          Élèves avec 5/5
        </p>
        <p class="text-xs text-gray-400">
          {{ qcmData.total_perfect_pct }}% des {{ header.n_candidates }} copies
        </p>
      </div>
      <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <AppIcon
        ...
        />
        <p class="text-2xl font-bold text-neutralDark">
          {{ qcmData.j1_perfect_count }}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          BB_J1 — {{ qcmData.j1_perfect_pct }}%
        </p>
        <p class="text-xs text-gray-400">
          QCM très accessible
        </p>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <AppIcon
          name="bar-chart-3"
          :size="20"
          class="text-purple-600 mb-2"
        />
        <p class="text-2xl font-bold text-neutralDark">
          {{ qcmData.j2_perfect_count }}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          BB_J2 — {{ qcmData.j2_perfect_pct }}%
        </p>
        <p class="text-xs text-gray-400">
          QCM discriminant
        </p>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 bg-green-50">
          <h2 class="text-base font-semibold text-green-800">
            BB_J1 — Distribution Ex1 (moy {{ qcmData.j1_mean_ex1 }}/5)
          </h2>
        </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 bg-purple-50">
          <h2 class="text-base font-semibold text-purple-800">
            BB_J2 — Distribution Ex1 (moy {{ qcmData.j2_mean_ex1 }}/5)
          </h2>
        </div>
          ...
          >
            ...
            <div class="flex-1 bg-gray-100 rounded-full h-5 overflow-hidden relative">
              <div
                ...
                :style="{ width: (d.count / maxDistJ2 * 100) + '%' }"
              />
              <span
                class="absolute inset-0 flex items-center justify-center text-xs font-semibold"
                :class="d.count >= 15 ? 'text-white' : 'text-gray-700'"
              >{{ d.count }} ({{ d.pct }})</span>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm mb-8 overflow-hidden">
      <div class="px-5 py-4 border-b border-gray-100">
        <h2 class="text-lg font-semibold text-neutralDark">
          {{ qcmData.total_perfect }} Élèves avec 5/5 à l'Exercice 1 (QCM)
        </h2>
        <p class="text-xs text-gray-400 mt-1">
          Classement par note globale décroissante · <span class="text-amber-600">Jaune</span> = %Ex1 &gt; 31.2% (seuil J2) · <span class="text-red-600">Rouge</span> = %Ex1 ≥ 50%
        </p>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50">
              <th class="text-center px-2 py-2.5 font-medium text-gray-600 w-10">
                #
              </th>
              ...
              <th
                v-if="hasGroups"
                class="text-center px-2 py-2.5 font-medium text-gray-600"
              >
                Grp
              </th>
              <th class="text-center px-2 py-2.5 font-medium text-gray-600">
          ...
          <tbody>
            <tr
              v-for="(s, i) in qcmPerfect"
              :key="i"
              class="border-t border-gray-100"
              :class="s.pct >= 50 ? 'bg-red-50' : s.pct > 31.2 && s.exam === 'BB_J1' ? 'bg-amber-50' : ''"
            >
              ...
              <td
                class="px-2 py-2 text-center font-bold"
                :class="s.total < 10 ? 'text-red-700' : 'text-gray-800'"
              >
                {{ s.total }}
              </td>
              <td
                class="px-2 py-2 text-center font-semibold"
                :class="s.pct >= 50 ? 'text-red-700' : s.pct >= 40 ? 'text-amber-700' : 'text-gray-600'"
              >
                {{ s.pct }}%
              </td>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 mb-4">
      ...
      <div>
        <p class="text-sm font-semibold text-amber-800">
          24 élèves BB_J1 avec %Ex1 supérieur au seuil BB_J2 (31.2%)
        </p>
        <p class="text-sm text-amber-700 mt-1">
          Les 16 élèves BB_J2 ayant eu 5/5 ont un %Ex1 moyen de 31.2%. <strong>24 élèves BB_J1 sur 54</strong> (44.4%) dépassent ce seuil, indiquant un poids disproportionné du QCM dans leur note. Les écarts les plus extrêmes atteignent +49 points (CHAMAM, KHALSI).
        </p>
      </div>
    </div>
    <div class="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3 mb-4">
      ...
      <div>
        <p class="text-sm font-semibold text-red-800">
          6 élèves avec 5/5 au QCM mais note globale &lt; 10/20
        </p>
        <p class="text-sm text-red-700 mt-1">
          Pour ces élèves, le QCM représente entre 50% et 80% de leur note totale. Sans le QCM, leur note effective serait entre 1.2 et 5.0 sur 15 points — <strong>fragilité extrême sur les exercices de rédaction</strong>.
        </p>
        <p class="text-sm text-red-700 mt-1">
          <strong>4 des 6 sont en T.01</strong> — confirmation de la difficulté structurelle de cette classe (taux réussite BB_J1 : 43.8%).
        </p>
      </div>
    </div>
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 mb-8">
      ...
      <div>
        <p class="text-sm font-semibold text-amber-800">
          Le QCM BB_J1 est très peu discriminant
        </p>
        <p class="text-sm text-amber-700 mt-1">
          50.9% des candidats obtiennent 5/5 à BB_J1 contre seulement 15.5% à BB_J2. Le QCM BB_J2 différencie nettement mieux les niveaux (moyenne 2.93/5 vs 4.12/5).
        </p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border-2 border-red-300 shadow-sm mb-8 overflow-hidden">
      <div class="px-5 py-4 border-b border-red-200 bg-red-50">
        ...
        <p class="text-xs text-red-600 mt-1">
          Pattern caractéristique : [0, 1, 0, 1, 0] → 2/5 — réponses du mauvais sujet
        </p>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-red-50/50">
              ...
              <th class="text-center px-1 py-2.5 font-medium text-gray-600">
                Q1.1
              </th>
              <th class="text-center px-1 py-2.5 font-medium text-gray-600">
                Q1.2
          ...
          <tbody>
            <tr
              v-for="(c, i) in cheatDetected"
              :key="i"
              class="border-t border-gray-100 bg-red-50/30 hover:bg-red-50"
            >
              <td class="px-2 py-2 text-center text-gray-500 text-xs">
                {{ i + 1 }}
              </td>
              ...
              <td class="px-2 py-2 text-center">
                <span
                  ...
                  :class="c.subject === 'B' ? 'bg-red-200 text-red-900' : 'bg-orange-100 text-orange-800'"
                >{{ c.subject }}</span>
              </td>
              <td class="px-2 py-2 text-center text-gray-600">
                {{ c.classe }}
              ...
              <td
                ...
                :class="c.q[0] === 0 ? 'text-red-600 font-bold' : 'text-green-600 font-bold'"
              >
                {{ c.q[0] }}
              </td>
              <td
                class="px-1 py-2 text-center"
                :class="c.q[1] === 0 ? 'text-red-600 font-bold' : 'text-green-600 font-bold'"
              >
                {{ c.q[1] }}
              </td>
              <td
                class="px-1 py-2 text-center"
                :class="c.q[2] === 0 ? 'text-red-600 font-bold' : 'text-green-600 font-bold'"
              >
                {{ c.q[2] }}
              </td>
              <td
                class="px-1 py-2 text-center"
                :class="c.q[3] === 0 ? 'text-red-600 font-bold' : 'text-green-600 font-bold'"
              >
                {{ c.q[3] }}
              </td>
              <td
                class="px-1 py-2 text-center"
                :class="c.q[4] === 0 ? 'text-red-600 font-bold' : 'text-green-600 font-bold'"
              >
                {{ c.q[4] }}
              </td>
              <td class="px-2 py-2 text-center font-bold text-red-700">
                2/5
              </td>
              <td
                class="px-2 py-2 text-center font-bold"
                :class="c.total < 10 ? 'text-red-700' : 'text-gray-800'"
              >
                {{ c.total }}
              </td>
              <td class="px-3 py-2 text-gray-500 text-xs">
      ...
      <div class="px-5 py-4 border-t border-red-200 bg-red-50">
        <p class="text-sm text-red-800">
          <strong>Asymétrie 9A vs 1B</strong> : les 9 élèves du Sujet A ont probablement recopié les réponses d'un voisin Sujet B. HAMZAOUI (Sujet B) a recopié les réponses d'un voisin Sujet A.
        </p>
        <p class="text-sm text-red-700 mt-1">
          Taux de triche potentielle : <strong>9.4%</strong> des candidats BB_J1. Note moyenne des tricheurs : <strong>10.09/20</strong>.
        </p>
      </div>
    </div>
  </div>
