...
**Règles** :
- Workers = CPU * 2 + 1 (règle générale)
- Timeout 30s (ajuster selon besoins)
- Max requests pour éviter memory leaks
- Logs vers stdout/stderr (Docker logs)
...
**Règles** :
- Wait for database avant migrations
- Migrations automatiques au démarrage
- Collectstatic si nécessaire
- `exec "$@"` pour signal propagation
...
- Security headers stricts
- Client max body size pour uploads
- Timeouts appropriés
- Cache pour static/media
...
- Service Certbot avec loop 12h
- Nginx reload après renouvellement
...
**Règles** :
- Logs JSON en production (parsing facile)
- Level INFO minimum
- Logs sécurité en WARNING
