# Règles de Supervision de Claude

## Statut : OBLIGATOIRE POUR TOUTE IA/DÉVELOPPEUR

Ces règles définissent comment Claude (ou tout développeur) doit utiliser le système de gouvernance `.claude/`.
...
Chaque réponse de Claude contenant du code doit inclure :
...
## ⛔ Règle n°2 — Toute PR Doit Citer `.claude/`
...
- Pas de section "Conformité .claude/" → ❌ REFUSÉ
...
## ⛔ Règle n°3 — Le `.claude/` Est la Source d'Autorité

### Principe

> En cas de conflit entre :
> - Intuition vs `.claude/` → `.claude/` gagne
> - Rapidité vs `.claude/` → `.claude/` gagne
> - "Solution facile" vs `.claude/` → `.claude/` gagne

### Hiérarchie des Décisions

```
1. `.claude/rules/` ← NON NÉGOCIABLE
2. `.claude/workflows/` ← OBLIGATOIRE
3. `.claude/skills/` ← GUIDE
...
Si Claude estime qu'une règle doit être modifiée :
...
**En aucun cas** : coder en contradiction avec `.claude/` actuel
...
Claude (ou reviewer) doit vérifier :

- [ ] Section "Conformité .claude/" présente dans chaque PR
...
Les seuls cas où `.claude/` peut être contourné :
...
2. **Bug bloquant production**
   ...
   - Conformité `.claude/` restaurée dans PR suivant
...
### Claude (IA Développeur)
- Consulter `.claude/` avant tout code
- Citer `.claude/` dans chaque PR
- Respecter hiérarchie des règles
- Proposer ADR si modification nécessaire

### Reviewer (Humain)
- Vérifier conformité `.claude/`
- Refuser PR non conforme
- Valider checklists
- Signaler incohérences dans `.claude/`

### Lead Technique
- Maintenir `.claude/` à jour
- Valider les ADR proposés
- Faire respecter les règles
- Former l'équipe sur `.claude/`
...
| Type | Sanction |
|------|----------|
| Code sans consultation `.claude/` | PR refusé, à retravailler |
...
2. **Deuxième violation** : Review obligatoire de `.claude/`
...
Le `.claude/` n'est pas une suggestion, c'est un **contrat**.

**Règle d'or** : Si vous hésitez, consultez `.claude/`. Si `.claude/` est ambigu, appliquez la règle la plus stricte.
