# 01.3 - Frontend Vue.js - Architecture Détaillée
...
│   ├── stores/             # Stores Pinia
│   ├── services/           # Services API et utils
...
**Fonctionnalités** :
- Vue d'ensemble des examens (en cours, terminés)
- Statistiques globales (copies corrigées, en attente)
- Accès rapide aux actions admin
...
<template>
  <div class="dashboard-admin">
    ...
    <div class="stats-grid">
      ...
      <StatCard title="Copies à Identifier" :value="stats.pending_copies" icon="clipboard" />
      <StatCard title="Copies Corrigées" :value="stats.corrected_copies" icon="check" />
...
**Rôle** : Formulaire de création d'examen

**Workflow** :
1. Saisir informations (nom, date)
2. Définir structure de notation (barème)
3. Uploader CSV étudiants
...
<template>
  <div class="exam-create">
    ...
    <!-- Étape 2: Structure de notation -->
    <section v-if="currentStep === 2">
      <h2>Structure de Notation (Barème)</h2>

      <div v-for="(question, idx) in exam.grading_structure" :key="idx" class="question-block">
        <div class="question-header">
          <input v-model="question.question" placeholder="Numéro (ex: 1)" />
          <input v-model="question.title" placeholder="Titre (ex: Étude de fonction)" />
        ...
        <div class="sub-questions">
          <div v-for="(sub, subIdx) in question.sub_questions" :key="subIdx" class="sub-question">
            ...
            <button @click="removeSubQuestion(idx, subIdx)" class="btn-sm">×</button>
          </div>
      ...
      <div class="actions">
        <button @click="previousStep" class="btn-secondary">Précédent</button>
    ...
    <section v-if="currentStep === 3">
      ...
      <div class="form-group">
        ...
        <input
          ...
          required
        />
        <small>Formats supportés: A3, A4</small>
      </div>
...
**Rôle** : Bureau d'identification pour copies non identifiées automatiquement
  ...
  <div class="identification-desk">
    ...
    <div v-if="selectedCopy" class="identification-panel">
      ...
      <div v-if="selectedCopy.ocr_mode === 'SEMI_AUTO'" class="ocr-candidates">
        <h3>🤖 Suggestions OCR</h3>
        <p class="help-text">
          Les moteurs OCR ont détecté ces candidats. Sélectionnez le bon étudiant :
        </p>
        ...
        >
          ...
          <div class="candidate-info">
            ...
            <p class="email">📧 {{ candidate.student.email }}</p>
            <p class="dob">🎂 {{ formatDate(candidate.student.date_of_birth) }}</p>
          </div>
          ...
          <div class="confidence-metrics">
            ...
            <div class="vote-info">
              ✓ {{ candidate.vote_count }} moteurs d'accord
            </div>
          </div>
        ...
        <button @click="showManualSearch = true" class="btn-manual">
          Aucun de ces étudiants ? Recherche Manuelle
        </button>
      </div>
...
#### CopyGrading.vue (CorrectorDesk)

**Rôle** : Interface de correction pour enseignants

**Fonctionnalités** :
- Visualisation PDF avec PDF.js
- **Mode split view** : affichage cote a cote PDF + panneau de notation (v2.2)
- Annotations (surlignage, commentaires)
- **Tampon rapide V/X** : outil TrueFalseTool.vue pour apposer VRAI / FAUX directement sur la copie (v2.2)
- Attribution des notes par question
- Sauvegarde automatique (draft)
- Verrouillage apres correction
- **Boutons admin** (v2.2, visibles uniquement pour staff/superuser) :
  - Force-unlock : deverrouillage force d'une copie verrouillee
  - Reopen : reouvrir une copie finalisee (GRADED -> READY)

**Composants V2.2** :
- `TrueFalseTool.vue` : outil tampon V/X, genere des annotations de type `VRAI` ou `FAUX`
- `CanvasLayer.vue` : rendu canvas des annotations, inclut le rendu des tampons V/X (icones check/cross colores)

**Types d'annotations supportes** :
- `HIGHLIGHT` : surlignage
- `COMMENT` : commentaire texte
- `VRAI` : tampon vert (v2.2)
- `FAUX` : tampon rouge (v2.2)
...
function autoSave() {
  // Debounce: sauvegarder 2s après dernière modification
...
async function finalize() {
  ...
  alert('Copie finalisée et verrouillée')
...
**Rôle** : Portail étudiant pour consultation des copies
...
Le frontend Vue.js de Korrigo fournit une interface utilisateur moderne et réactive pour tous les profils (admin, enseignant, étudiant).
