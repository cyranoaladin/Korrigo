# 01.2 - Backend Django - Architecture Détaillée
...
Le backend Korrigo est construit sur **Django 4.2.27** et **Django REST Framework 3.16.1**, fournissant une API REST complète pour la gestion des examens, la correction numérique et l'identification OCR.

### Architecture en Applications Django

Le backend est organisé en **6 applications Django** principales :

```
backend/
├── core/           # Authentification, utilisateurs, permissions
├── exams/          # Gestion des examens et copies
├── grading/        # Correction et notation
├── students/       # Gestion des étudiants et résultats
├── identification/ # OCR et identification des copies
└── processing/     # Traitement batch PDF et OCR
```

---

## 📦 Application `core/` - Authentification et Base
...
class User(AbstractUser):
    """Utilisateur Django étendu."""

    # Hérite de AbstractUser (username, email, password, etc.)
...
## 📝 Application `exams/` - Gestion des Examens

### Responsabilités

- CRUD des examens
- Upload des PDF scannés
- Structure de notation (barème)
...
class Copy(models.Model):
    ...
    # Correction
    is_locked = models.BooleanField(default=False)  # Verrouillée après correction
...
class Page(models.Model):
    """Page scannée d'une copie."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    copy = models.ForeignKey(Copy, on_delete=models.CASCADE, related_name='pages')

    page_number = models.IntegerField()  # Numéro de page dans la copie
...
## ✅ Application `grading/` - Correction et Notation
...
class Grade(models.Model):
    """Note attribuée pour une question."""
...
class CopyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Filtrer les copies selon le rôle.

        - Admin: toutes les copies
        - Teacher: copies assignées
        """
            ...
            [
                {
                    ...
                    "exam": {"id": "...", "name": "..."},
                    "anonymous_id": "A3-001",
                    "student": null,  // null si pas encore identifié
                    "status": "READY",
...
class CopyViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, pk=None):
        """
        Mettre à jour une copie (notes, commentaires, annotations).

        Body:
            {
                "grades": [
                    {"question_id": "1", "score": 4.5, "max_score": 5.0, "comment": "Très bien"},
        ...
        # Vérifier que la copie n'est pas verrouillée
        if copy.is_locked:
...
class CopyViewSet(viewsets.ModelViewSet):
    ...
    def lock(self, request, pk=None):
        """
        Verrouiller une copie après correction.
        ...
        # Vérifier que toutes les questions sont notées
...
## 👨‍🎓 Application `students/` - Gestion des Étudiants

### Responsabilités

- CRUD des étudiants
- Authentification étudiants (email + date de naissance)
- Consultation des résultats
- Téléchargement des copies corrigées
...
def student_login(request):
    """
    Authentification étudiant (email + date de naissance).
...
def student_copies(request):
    """
    Liste des copies de l'étudiant connecté.
...
## 🔍 Application `identification/` - OCR et Identification

### Responsabilités

- OCR des en-têtes de copies
- Matching étudiant avec CSV (fuzzy matching)
...
class OCRResult(models.Model):
    ...
    # Top-K candidats (nouveau Production)
...
def get_ocr_candidates(request, copy_id):
    """
    Récupérer les top-K candidats OCR pour sélection semi-automatique.
...
def select_ocr_candidate(request, copy_id):
    """
    Sélectionner un candidat parmi les top-K (mode SEMI_AUTO).
...
## ⚙️ Application `processing/` - Traitement Batch et OCR

### Responsabilités

- Traitement batch des PDF A3/A4
- Split des pages A3 en A4
- Segmentation par étudiant (multi-sheet fusion)
...
class BatchA3Processor:
    ...
    def process(self, pdf_path: str, csv_path: str) -> ProcessingResult:
        ...
        Étapes:
        1. Détection format (A3 vs A4)
        2. Split A3 → A4 (avec rotation si nécessaire)
        3. OCR GPT-4o-mini Vision sur en-têtes
        4. Segmentation par étudiant (multi-sheet fusion)
        5. Création des copies Django

        Args:
            pdf_path: Chemin vers le PDF source
            csv_path: Chemin vers le CSV des étudiants
        ...
        # 2. Détecter format et split si A3
    ...
    def _split_a3_pages(self, pages: List[PageInfo]) -> List[PageInfo]:
        ...
        Logique:
        - A3 portrait → 2 A4 landscape (rotation 90°)
    ...
    def _segment_by_student(self, pages: List[PageInfo],
        ...
        """
        Segmentation par étudiant avec multi-sheet fusion.

        Logique:
        - Détecter header (vision.py)
        - OCR GPT-4o-mini Vision
        - Grouper les pages par étudiant (fusion 2+ sheets)
        """
        student_copies = []
        current_copy = None
        ...
        for page in pages:
            ...
            if header_region is not None:
                # Nouveau header détecté → nouvelle copie
    ...
    def _ocr_header_multi_layer(self, header_img: np.ndarray) -> OCRResult:
        ...
        # Déterminer mode selon confidence
...
class MultiLayerOCR:
    ...
    def _consensus_vote(self, ocr_results: List[OCRCandidate],
        ...
        """
        Voting par agrégation des scores OCR + fuzzy matching.

        Algorithme:
        - Chaque OCR candidate → fuzzy match contre CSV
        - Agréger les scores par étudiant
        - Retourner top-5
        """
...
def process_batch_pdf(self, exam_id: str, pdf_path: str,
    ...
    Progress:
        Met à jour self.update_state() pour tracking frontend
    """
...
CSRF_COOKIE_HTTPONLY = False  # Pour accès JavaScript
...
# ❌ UNSAFE - Raw SQL non paramétré
cursor.execute(f"SELECT * FROM users WHERE username='{user_input}'")

# ✅ SAFE - Raw SQL paramétré
...
class RateLimitMiddleware:
    def __call__(self, request):
        ...
        cache.set(key, count + 1, 60)  # Expire après 60s
...
Le backend Django de Korrigo fournit une architecture robuste et modulaire pour la gestion complète des examens numériques, de la création à la consultation des résultats.

**Points clés** :
- Architecture en 6 applications Django bien séparées
- API REST complète avec DRF
- OCR GPT-4o-mini Vision pour identification robuste
