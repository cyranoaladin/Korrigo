...
| Déverrouiller | ✅ | ❌ | ❌ |
| Force-unlock (v2.2) | ✅ (staff/superuser) | ❌ | ❌ |
| Reopen GRADED->READY (v2.2) | ✅ (superuser) | ❌ | ❌ |
...
### 6. POST /api/grading/copies/{id}/force-unlock/ (v2.2)

**Description** : Deverrouillage force d'une copie verrouillee par un autre correcteur

**Permissions** : Staff ou Superuser uniquement (`is_staff=True`)

**Cas d'usage** : Un correcteur a verrouille une copie mais n'est plus disponible ; l'admin force le deverrouillage pour permettre la reassignation ou la reprise de correction.
...
```json
{
  ...
  "message": "Copy force-unlocked successfully"
}
```

**Reponse Error (403 Forbidden)** :

```json
{
  "error": "Staff or superuser required"
}
```

---

### 7. POST /api/grading/copies/{id}/reopen/ (v2.2)

**Description** : Reouvrir une copie finalisee (transition GRADED -> READY) pour permettre une re-correction

**Permissions** : Superuser uniquement (`is_superuser=True`)

**Cas d'usage** : Une copie a ete finalisee (status GRADED/COMPLETED) mais une erreur de correction a ete detectee ; le superuser reouvre la copie pour permettre une nouvelle correction.

**Transition de statut** : `GRADED` / `COMPLETED` -> `READY`
...
```json
{
  ...
  "message": "Copy reopened successfully (GRADED -> READY)"
}
```
...
**Reponse Error (400 Bad Request)** :

```json
{
  "error": "Copy is not in GRADED/COMPLETED status"
}
```

---

### 8. GET /api/grading/copies/{id}/pdf/
...
### 9. GET /api/grading/copies/{id}/statistics/
...
- Force-unlock pour deverrouillage admin (v2.2)
- Reopen pour reouvrir copie finalisee (v2.2)
- Transition GRADED -> READY (v2.2)
