...
L'authentification Korrigo repose sur un système de **sessions Django** avec protection **CSRF**. Les utilisateurs s'authentifient une fois et reçoivent un **cookie de session** valable 24 heures.

### Méthode d'Authentification

- **Backend** : Django Session Authentication
- **Frontend** : Axios avec `withCredentials: true` (cookies automatiques)
- **Protection** : CSRF Token + CORS configuré
- **Durée session** : 24 heures (configurable)
...
**Description** : Authentifier un utilisateur (admin, enseignant, étudiant)
...
- `csrftoken` : Token CSRF pour requêtes suivantes
...
async function login(username: string, password: string) {
  ...
  if (response.data.success) {
    // Cookies sessionid et csrftoken sont automatiquement sauvegardés
...
**Description** : Déconnecter l'utilisateur (invalider session)
...
async function logout() {
  ...
  // Session invalidée, cookies supprimés par navigateur
}
```
...
def me_view(request):
    """
    Récupérer informations utilisateur connecté.
...
SESSION_COOKIE_HTTPONLY = True  # Pas accessible en JS (sécurité)
...
## 👤 Authentification Étudiants (Alternative)

### POST /api/students/login/

**Description** : Authentification étudiants via email + date de naissance
...
def student_login(request):
    """
    Authentification étudiant (email + date naissance).
...
class IsOwnerOrAdmin(BasePermission):
    """Permission pour propriétaire de la ressource ou admin."""

    def has_object_permission(self, request, view, obj):
        if request.user.is_superuser:
            return True

        # Vérifier propriétaire selon type d'objet
...
router.beforeEach(async (to, from, next) => {
  ...
  // Charger utilisateur si pas déjà fait
...
L'API d'authentification Korrigo fournit un système de sessions sécurisé avec protection CSRF et gestion des rôles.
...
- Support authentification alternative pour étudiants
