...
L'API Identification permet aux **enseignants et administrateurs** de gérer l'identification des copies via OCR (modes AUTO, SEMI-AUTO, MANUAL).
...
| **SEMI-AUTO** | 0.4 - 0.7 | Sélection parmi top-5 candidats OCR |
| **MANUAL** | < 0.4 | Recherche manuelle complète |
...
**Description** : Lister les copies non identifiées (bureau d'identification)
...
class IdentificationViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        # Filtrer copies non identifiées
...
**Description** : Récupérer les top-5 candidats OCR (mode SEMI-AUTO)
  ...
  "candidates": [
    ...
    },
    {
      ...
      "student": {
        ...
        "first_name": "Jeanne",
        "last_name": "DUPOND",
        "email": "jeanne.dupond@viatique.local",
...
def ocr_candidates(self, request, pk=None):
    """Récupérer candidats OCR pour sélection."""
...
**Description** : Sélectionner un candidat parmi les top-5 (mode SEMI-AUTO)
...
**Description** : Assigner manuellement un étudiant (mode MANUAL)
...
def manual_assign(self, request, pk=None):
    ...
    # Vérifier que étudiant appartient au même examen
...
def _get_exam_students_emails(self, exam):
    """Récupérer emails étudiants du CSV."""
...
**Description** : Rechercher étudiants (pour assignation manuelle)

**Permissions** : Admin et Teacher uniquement

**Query Parameters** :
- `q` (requis) : Requête de recherche (nom, prénom, email)
- `exam_id` (optionnel) : Filtrer étudiants du CSV de cet examen
...
def search_students(self, request):
    ...
    # Filtrer par examen si spécifié
    exam_id = request.query_params.get('exam_id')
    if exam_id:
...
# Rechercher dans examen spécifique
...
**Description** : Forcer ré-identification (relancer OCR)
...
def force_reidentify(self, request, pk=None):
    """Forcer ré-identification (admin uniquement)."""
...
L'API Identification fournit les outils nécessaires pour gérer l'identification des copies avec support des 3 modes (AUTO, SEMI-AUTO, MANUAL).

**Points clés** :
- Top-5 candidats OCR avec sources
- Sélection assistée (SEMI-AUTO)
