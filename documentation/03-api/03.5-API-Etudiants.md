# 03.5 - API Étudiants (Portail Élève)
...
L'API Étudiants permet aux élèves de :
- Se connecter avec email + date de naissance
- Consulter leurs copies corrigées
- Télécharger leurs résultats PDF
...
Les étudiants utilisent un **système d'authentification alternatif** sans compte utilisateur Django :
- **Login** : Email + Date de naissance (pas de mot de passe)
- **Session** : Cookie Django avec `student_id`
- **Permissions** : Accès limité à leurs propres copies
...
Authentifie un étudiant avec email + date de naissance.
...
// Session cookie automatiquement stocké
```

---

### 2. Récupérer Profil Étudiant
...
### 3. Lister Copies Élève

**GET** `/api/students/copies/`

Liste toutes les copies de l'étudiant connecté (publiées uniquement).
  ...
  "results": [
    {
      ...
      "exam": {
        "id": 5,
        "title": "Évaluation Loi Binomiale",
...
### 4. Récupérer Détails Copie

**GET** `/api/students/copies/{id}/`

Récupère les détails d'une copie avec notes par question.

#### Permissions
- **IsStudent** (copie appartient à l'étudiant)
...
```json
{
  ...
  "grades": [
    {
      "question_number": 1,
      "question_title": "Calcul de probabilité",
      "score": 4.0,
      "max_score": 5.0,
    ...
    {
      "question_number": 3,
      "question_title": "Exercice synthèse",
      "score": 6.5,
      "max_score": 10.0,
...
| Code | Raison |
|------|--------|
| 403 | Copie non publiée ou n'appartient pas à l'étudiant |
...
### 5. Télécharger PDF Copie

**GET** `/api/students/copies/{id}/pdf/`

Télécharge le PDF de la copie corrigée.

#### Permissions
- **IsStudent** (copie publiée uniquement)
...
### 6. Statistiques Élève

**GET** `/api/students/statistics/`

Récupère les statistiques globales de l'étudiant.
...
```json
{
  ...
  "recent_exams": [
    {
      "exam_title": "Évaluation Loi Binomiale",
      "date": "2026-01-20",
      "score": 15.5,
      "percentage": 77.5,
      "class_average": 14.2
...
Déconnecte l'étudiant (supprime session).
...
def student_profile(request):
    """Récupérer profil étudiant connecté."""
...
class StudentCopyViewSet(viewsets.ReadOnlyModelViewSet):
    """
    ViewSet pour consultation copies par étudiants.

    Permissions:
    - Lecture seule
    - Copies publiées uniquement
    - Uniquement copies de l'étudiant connecté
    """
    serializer_class = StudentCopySerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        """Filtrer copies de l'étudiant connecté."""
        student_id = self.request.session.get('student_id')

        if not student_id:
            return Copy.objects.none()

        # Uniquement copies publiées
    ...
    def retrieve(self, request, pk=None):
        """Récupérer détails copie avec notes."""
...
class IsStudent(BasePermission):
    ...
    def has_permission(self, request, view):
        """Vérifier que user est étudiant (session)."""
        return bool(
            request.session and
...
class StudentSerializer(serializers.ModelSerializer):
    """Serializer pour profil étudiant."""
...
class StudentCopyGradeSerializer(serializers.ModelSerializer):
    """Serializer pour notes visibles par étudiant."""
...
class StudentCopySerializer(serializers.ModelSerializer):
    """Serializer pour copies visibles par étudiant."""
...
async function handleLogin() {
  ...
  try {
    ...
    error.value = err.response?.data?.error || 'Connexion échouée'
  } finally {
    loading.value = false
  }
}
    ...
    <section class="copies-list">
      ...
      <div v-for="copy in copies" :key="copy.id" class="copy-card">
        ...
        <div class="copy-actions">
          <button @click="viewCopy(copy.id)">Voir Détails</button>
...
**Risques** :
- ⚠️ Moins sécurisé qu'un mot de passe
- ⚠️ Date de naissance = information publique potentielle

**Mitigations** :
- ✅ Session courte (2 heures)
- ✅ Accès lecture seule (pas de modification)
- ✅ Copies publiées uniquement
- ✅ Rate limiting sur endpoint login
...
class StudentAuthTestCase(TestCase):
    ...
    def test_access_profile_after_login(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['email'], 'jean.dupont@korrigo.local')
```
...
L'API Étudiants offre un portail simple et sécurisé pour consulter les résultats. L'authentification email + date de naissance évite la gestion de mots de passe tout en restant suffisamment sécurisée pour un contexte éducatif.

**Points clés** :
- Authentification alternative sans compte utilisateur
- Accès lecture seule aux copies publiées
