...
Ce workflow décrit le processus complet d'**upload et traitement des PDF scannés** :
1. Upload PDF + CSV
2. Validation fichiers
3. Traitement batch (split A4, OCR, segmentation)
4. Création des copies Django
5. Prêt pour identification
...
┌──────────────────┐
│  Validation      │
│  - Format PDF    │ ◄─── Si erreur: 400 Bad Request
│  - Taille < 100MB│
│  - CSV valide    │
└──────┬───────────┘
       │
...
┌──────────────────┐
│  Celery Task     │
│  Async Processing│ ◄─── Task ID retourné
└──────┬───────────┘
       │
       ├─────────────────────────────┐
       │                             │
...
**Actions** :
1. Sélectionner PDF scanné (copie complète agrafée)
2. Sélectionner CSV liste étudiants
...
<template>
  <div class="upload-form">
    <h2>📤 Upload Copies Scannées</h2>

    <form @submit.prevent="handleUpload">
      <!-- PDF Upload -->
      <div class="form-group">
        <label>📄 PDF Scanné (copies agrafées)</label>
        <input
          type="file"
          accept=".pdf"
          @change="onPDFChange"
      ...
      <!-- CSV Upload -->
      <div class="form-group">
        <label>📊 CSV Liste Étudiants</label>
        <input
          type="file"
          accept=".csv"
          @change="onCSVChange"
      ...
      <div class="form-group">
        ...
        <select v-model="batchMode" required>
          <option value="A3">A3 (recto-verso agrafé)</option>
...
async function handleUpload() {
  if (!pdfFile.value || !csvFile.value) {
    alert('Veuillez sélectionner PDF et CSV')
    return
  }
  ...
  try {
    ...
    alert(error.response?.data?.error || 'Upload échoué')
  } finally {
...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    def upload(self, request):
        ...
        exam = Exam.objects.create(
            ...
            subject=request.data.get('subject', 'Non spécifié'),
...
### Étape 3 : Traitement Batch Async (Celery)
...
1. **Split A4** : Convertir A3 → 2x A4 avec rotation
2. **OCR Header** : Extraire nom + date de naissance (multi-layer)
3. **Segmentation** : Grouper pages par étudiant
4. **Fusion Sheets** : Fusionner feuilles multi-pages
5. **Matching CSV** : Fuzzy match avec whitelist
6. **Création Copies** : Sauvegarder dans Django

---

### Étape 4 : Split A4 (Batch A3)

**Service** : `BatchA3Processor._split_a3_to_a4()`

**Logique** :

- Page A3 impaire (recto) → split vertical → [A4 gauche, A4 droit]
- Page A3 paire (verso) → split vertical + rotation 180° → [A4 gauche, A4 droit]
...
class BatchA3Processor:
    ...
    def _split_a3_to_a4(self, pdf_path: str) -> List[PageInfo]:
        ...
        Returns:
            Liste de PageInfo (numéro page, image, orientation)
        """
        ...
        for a3_page_num in range(len(doc)):
            ...
            # Page RECTO (impaire) → split normal
            if a3_page_num % 2 == 0:
        ...
        doc.close()
        logger.info(f"Split {len(doc)} A3 pages → {len(a4_pages)} A4 pages")
...
### Étape 5 : OCR Header Multi-Layer

**Service** : `BatchA3Processor._ocr_header()`

**Moteurs OCR Utilisés** :

1. **Tesseract** (PSM 6, French) → Bon pour texte imprimé
2. **EasyOCR** (fr, en) → Bon pour écriture manuscrite
3. **PaddleOCR** (lang=fr) → Robuste et rapide

**Consensus Voting** :

- Chaque moteur OCR extrait texte
- Fuzzy match avec CSV whitelist
- Agrégation scores → Top-5 candidats
- Confiance > 0.7 → AUTO
- Confiance 0.4-0.7 → SEMI-AUTO (top-5)
- Confiance < 0.4 → MANUAL
...
def _ocr_header(self, header_image: np.ndarray) -> OCRResult:
    """
    OCR GPT-4o-mini Vision sur header.

    Returns:
        OCRResult avec top_candidates
    """
    # Instancier OCR GPT-4o-mini Vision
...
### Étape 6 : Segmentation par Étudiant

**Service** : `BatchA3Processor._segment_by_student()`

**Logique** :

- Détecter headers (zone 5 cm haut de page)
- Grouper pages entre 2 headers = 1 copie étudiant
- Gérer multi-feuilles (ex: 4 pages par étudiant)
...
def _segment_by_student(self, a4_pages: List[PageInfo]) -> List[StudentCopy]:
    """
    Segmenter pages A4 par étudiant.
    ...
    logger.info(f"Segmented {len(a4_pages)} pages → {len(student_copies)} student copies")
...
def _create_student_copy(self, pages: List[PageInfo],
    ...
    """
    Créer StudentCopy avec OCR du premier header.
    """
    ...
    if ocr_result.top_candidates:
        ...
        if top_confidence > 0.7:
            ...
            ocr_mode = 'SEMI_AUTO'
            student_match = None  # Attendre sélection manuelle
        else:
            ocr_mode = 'MANUAL'
            student_match = None
    else:
...
### Étape 7 : Création Copies Django
...
- `OCRResult` : Résultats OCR avec top_candidates
...
def create_copies_from_batch(self, exam: Exam,
    ...
    """
    Créer copies Django depuis StudentCopy.

    Returns:
        Nombre de copies créées
    """
...
### Étape 8 : Notification Enseignant

**WebSocket** : Notification temps réel via Django Channels
...
|----------|--------|-------|
| **Upload** | ~5s pour 50 MB | Dépend connexion |
| **Split A3 → A4** | ~2s pour 88 pages | PyMuPDF rapide |
| **OCR par page** | ~3-5s | GPT-4o-mini Vision (3 moteurs) |
| **Segmentation** | ~1s pour 176 pages | Header detection |
| **Création Django** | ~10s pour 44 copies | I/O database |
| **Total** | **~8-12 min** pour 88 A3 pages (28 étudiants) | Batch complet |

### Optimisations

1. **Celery Worker Pool** : 4 workers en parallèle
2. **OCR Caching** : Éviter re-OCR si déjà traité
3. **Bulk Insert Django** : `bulk_create()` au lieu de boucle
4. **Redis Cache** : Résultats intermédiaires
...
Le workflow d'upload et traitement des scans est entièrement automatisé via Celery. Le système supporte des PDF volumineux (100 MB) et traite efficacement les formats A3 recto-verso avec OCR GPT-4o-mini Vision.
