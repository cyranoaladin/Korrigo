...
Ce document décrit la **configuration complète de Nginx** pour le système Korrigo, incluant reverse proxy, SSL, caching, rate limiting, et optimisations performance.
...
1. **Reverse Proxy** : Redirection requêtes → Gunicorn (Django)
...
5. **Caching** : Cache HTTP réponses API
...
│   └── options-ssl-nginx.conf  # Options SSL
├── conf.d/                 # Configurations supplémentaires
│   ├── gzip.conf
│   ├── security.conf
│   └── cache.conf
└── sites-available/
    └── korrigo.conf        # Configuration site
```
...
server {
    ...
    location /api/ {
        ...
        # Cache GET requests (uniquement endpoints spécifiques)
        ...
        # Cache bypass pour requêtes authentifiées
...
### Génération Diffie-Hellman Parameters

```bash
# Générer dhparam (peut prendre plusieurs minutes)
openssl dhparam -out infra/nginx/ssl/dhparam.pem 4096
```

### Options SSL (fichier séparé)
...
### 1. Rate Limiting Détaillé

**Zones définies** :

| Zone | Rate | Burst | Usage |
|------|------|-------|-------|
| `general` | 10 req/s | 20 | Requêtes générales |
...
# Réponse attendue après 5 requêtes:
...
geo $blocked_ip {
    default 0;
    192.168.1.100 1;  # IP à bloquer
    203.0.113.0/24 1;  # Subnet à bloquer
}
...
map $http_user_agent $bad_bot {
    default 0;
    ~*bot 0;  # Autoriser bots légitimes
    ~*BadBot 1;
    ~*SemrushBot 1;
    ~*AhrefsBot 1;
}
...
La configuration Nginx optimise les performances, la sécurité et la fiabilité du système Korrigo. Le reverse proxy gère SSL, rate limiting, caching, et compression pour offrir une expérience utilisateur optimale.

**Points clés** :
- HTTP/2 + SSL/TLS 1.3
- Rate limiting multi-zones
- Caching stratégique (static, API)
